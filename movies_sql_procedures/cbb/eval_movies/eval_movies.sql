...
CREATE OR REPLACE PACKAGE BODY EVAL_PACKAGE AS 
    ...
    (
        ...
        BEGIN
            -- vérifier disponibilité de CB
            BEGIN
                SELECT MIN(IdUser) INTO v_check FROM CB_USERS;
                -- si réussite, mise à jour CB + signaler disponibilité de CB via exception
                RESTORE_PACKAGE.RestoreDatabase();
                RAISE_APPLICATION_ERROR(-20400, 'CB disponible');
            EXCEPTION
                WHEN NO_DATA_FOUND THEN -- si réussite, mise à jour CB + signaler disponibilité de CB via exception
                    RESTORE_PACKAGE.RestoreDatabase();
                    RAISE_APPLICATION_ERROR(-20400, 'CB disponible');
                WHEN OTHERS THEN
                    IF SQLCODE = -28000 THEN
                        NULL; -- si inaccessible, continuer
                    ELSE
                        RAISE;
                    END IF;
                NULL; 
